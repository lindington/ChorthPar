...
t1file <- read.csv(paste("../../../inputs/bamlist_alp_gom.txt",sep = ""), header = FALSE)
t2file <- read.csv(paste("../../../inputs/bamlist_alp_tar.txt",sep = ""), header = FALSE)
  
t1n <- length(t1file$V1)
t2n <- length(t2file$V1)
  
#read in filenames of gene SFS
filelist <- list.files(paste0('../../02.sfs/03.output/02.big',t))
  
#Sum gene SFS into total SFS and clean up.
all_sfs <- t(sapply(filelist, function(x) scan(paste0('../../02.sfs/03.output/02.big',t,'/',x)), simplify = TRUE))
...
for (b in 1:100) {
  ...
  sink(paste0('../03.boots/02.big',t,'/boot',b,'.sfs'), append = FALSE)
  cat(paste0((2*t1n)+1,' ',(2*t2n)+1,' unfolded "',t1,'" "',t2,'"'))
  cat('\n')
  sink()
   
  write(boot_sfs, file = paste0('../03.boots/02.big',t,'/boot',b,'.sfs'), ncolumns = length(boot_sfs), append = TRUE, sep = " ")
