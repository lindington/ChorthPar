#ZN: I've made some changes to this script from the original published by Portik et al. 2017. Any changes I make will have a comment with my initials. Please cite Portik et al. 2017 when using this script.
...
def write_log(outfile, model_name, rep_results, roundrep):
    ...
    fh_log.write("\n{}\n".format(roundrep))
	#ZN: Changed templogname so that it is independent per run. This prevents any problems that could occur from parallel runs trying to write and read from it at the same time. Also moves it to a temp folder so that it can be easily hidden away from the rest of the files.
    templogname = "temp/{0}.{1}.log.txt".format(outfile, model_name)
    try:
...
def Optimize_Routine(fs, pts, outfile, model_name, func, rounds, param_number, fs_folded=True, reps=None, maxiters=None, folds=None, in_params=None, in_upper=None, in_lower=None, param_labels=" "):
    ...
    #ZN: Changed the mode from a (append) to w (write). This means that the old results files will be overwritten when a new analysis starts rather than appending new results to old ones. Also added a line to initialize the log file
    fh_out = open(outname, 'w')
    fh_out.write("Model"+'\t'+"Replicate"+'\t'+"log-likelihood"+'\t'+"AIC"+'\t'+"chi-squared"+'\t'+"theta"+'\t'+"optimized_params({})".format(param_labels)+'\n')
    fh_out.close()
	
    fh_log = open("{0}.{1}.log.txt".format(outfile, model_name), 'w')
    fh_log.close()
    ...
    for r in range(rounds):
        ...
        for rep in range(1, (reps_list[r]+1) ):
			...
			#ZN: Updated the output file name to the new templogfile name.
            params_opt = dadi.Inference.optimize_log_fmin(params_perturbed, fs, func_exec, pts, lower_bound=lower_bound, upper_bound=upper_bound, verbose=1, maxiter=maxiters_list[r], output_file = "temp/{0}.{1}.log.txt".format(outfile, model_name))
    ...
    #ZN: Adding this line to make the function return the maximum likelihood and best parameters for use in bootstrapping.
    final = []
    final.append(results_list[0][1])
    final.append(results_list[0][5].tolist())
    final.append(results_list[0][2])
    final.append(results_list[0][4])
    return final
	
    #cleanup file
	#ZN: Updated the filename to the new templogfile name so it is properly deleted.
    os.remove("temp/{0}.{1}.log.txt".format(outfile, model_name))
